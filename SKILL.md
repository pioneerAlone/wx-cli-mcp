...
3. Windows 用户：`wx-mcp.exe` 需在 PATH 中（install.ps1 会自动添加）

---

## 安装

### 方式一：npm（推荐，需要 Node.js >= 14）
...
**Windows 下 `wx-mcp` 找不到**：用绝对路径配置，例如 `C:\Users\<用户名>\AppData\Local\Programs\wx-mcp\wx-mcp.exe`。
