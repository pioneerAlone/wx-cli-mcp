...
3. 已全局安装 wx-mcp（`npm install -g @bakewell/wx-mcp`，然后验证 `wx-mcp --version`）

> ⚠️ **多 Agent 并发注意事项**：当多个 AI agent（如 Copilot CLI + Claude Code）同时配置 wx-mcp 时，请务必使用全局安装（`npm install -g`）方式，**不要使用 `npx -y`** 方式。`npx -y` 每次启动时会重新解析/下载包，多个 agent 并发启动时会产生 npm 缓存竞争，导致连接不稳定或超时。
...
> **说明**：推荐使用全局安装（`npm install -g @bakewell/wx-mcp`）方式配置，稳定性更好，尤其在多 agent 并发场景下。更新时运行 `npm install -g @bakewell/wx-mcp@latest`。

---

## 工具列表

wx-mcp 向 AI agent 暴露以下 16 个工具，覆盖微信数据的全部核心功能。
...
列出指定会话中的附件消息（图片、视频、文件等），支持按类型和时间范围过滤。
...
- `since`（可选）：起始时间 Unix timestamp
- `until`（可选）：结束时间 Unix timestamp
- `kinds`（可选）：附件类型过滤，字符串数组（当前支持 `"image"`）
...
# 只看"项目群"的图片附件
工具: wx_attachments
参数: { "chat": "项目群", "kinds": ["image"] }

# 查看本月"工作群"的图片附件（最多 100 条，从第 50 条开始）
工具: wx_attachments
参数: { "chat": "工作群", "kinds": ["image"], "since": 1748736000, "limit": 100, "offset": 50 }
```
...
将指定附件提取/解密到本地文件系统。附件 ID 可从 `wx_attachments` 的返回结果中获取。

**参数：**
- `attachment_id`（必填）：附件 ID（从 `wx_attachments` 返回结果中获取）
- `output`（必填）：输出文件的绝对路径（由 daemon 直接写入本地磁盘）
- `overwrite`（可选，默认 false）：是否覆盖已存在的文件

```
# 提取附件到指定路径
工具: wx_extract
参数: { "attachment_id": "abc123", "output": "/Users/me/Downloads/photo.jpg" }

# 强制覆盖已存在的文件
工具: wx_extract
参数: { "attachment_id": "abc123", "output": "/Users/me/Downloads/photo.jpg", "overwrite": true }
```
...
**Windows 下 `wx-mcp` 找不到**：确认已运行 `npm install -g @bakewell/wx-mcp`，然后重开终端让 PATH 生效。如仍找不到，可在 agent 配置中指定完整路径，如 `"command": "C:\\Users\\<用户名>\\AppData\\Roaming\\npm\\wx-mcp.cmd"`。
